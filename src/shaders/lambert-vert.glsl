...
void main()
{
    ...
    float freq = 7.0;  
    float noiseVal = noise(freq * vs_Pos.xyz);

    vec4 final_Pos = vs_Pos;
    final_Pos.y += sin(0.01 * u_Time);
    
    vec4 modelposition = u_Model * final_Pos;   // Temporarily store the transformed vertex positions for use below
